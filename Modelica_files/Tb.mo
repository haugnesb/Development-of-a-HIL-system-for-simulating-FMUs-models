...
model Turbine "Simple turbine model with mechanical connectors"
  ...
  parameter Modelica.Units.SI.Height H_n=460 "Nominal net head" annotation (
      Dialog(group="Nominal turbine parameters", enable=not ValveCapacity));
  parameter Modelica.Units.SI.VolumeFlowRate Vdot_n=23.4 "Nominal flow rate"
    annotation (Dialog(group="Nominal turbine parameters", enable=not
          ValveCapacity));
  parameter Modelica.Units.SI.PerUnit u_n=0.95 "Nominal guide vane opening"
    annotation (Dialog(group="Nominal turbine parameters", enable=not
          ValveCapacity));
  ...
  parameter Modelica.Units.SI.Efficiency eta_h=0.9
  ...
  extends OpenHPL.ElectroMech.BaseClasses.Power2Torque(power(y=Wdot_s), inertia(
        w(fixed=true)));
  extends OpenHPL.Interfaces.TurbineContacts2;

  Modelica.Units.SI.Pressure dp "Turbine pressure drop";
  Modelica.Units.SI.EnergyFlowRate Kdot_i_tr "Kinetic energy flow";
  Modelica.Units.SI.VolumeFlowRate Vdot "Flow rate";
  Real C_v_ "Guide vane 'valve capacity'";

  output Modelica.Units.SI.EnergyFlowRate Wdot_s "Shaft power";
...
<img src=\"modelica://OpenHPL/Resources/Images/turbinepic.svg\">
          ...
          fillPattern=FillPattern.Solid,
          textString="GVO")}),
    uses(OpenHPL(version="2.0.1"), Modelica(version="4.0.0")));
end Turbine;
